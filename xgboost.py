...
def objective(params):
    ...
    model = xgb.train(params, dm1, num_round, watchlist, maximize=True, early_stopping_rounds=20, verbose_eval=100)
...
train=pd.read_csv("./Train_without_Data_balancing.csv")
...
watchlist = [(dm1, 'train'), (dm2, 'valid')]
model = xgb.train(final_pera, dm1, num_round, watchlist, maximize=True, early_stopping_rounds=20, verbose_eval=100)
pred = model.predict(dm2, ntree_limit=model.best_ntree_limit)
auc = roc_auc_score(dm2.get_label(), pred)
print(f"SCORE: {auc}")

output(model,'./Test_without_Data_balancing.csv','./xg_predictions.csv')